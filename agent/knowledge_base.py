...
KB: Dict[str, Dict[str, object]] = {
    "hardcoded secret": {
        "summary": (
            "Secrets such as API keys and passwords must not be stored in source "
            "code or committed to version control. Use secret managers and "
            "environment variables instead."
        ),
        "examples": ["API_KEY = 'abcd1234'", "password = 'hunter2'"],
        "fix": (
            "Move secrets to environment variables or a secrets manager like "
            "AWS Secrets Manager or Hashicorp Vault. Use scoped, short-lived "
            "credentials."
        ),
        "references": [
    ...
    "insecure function usage": {
        "summary": (
            "Dynamic code execution functions (eval/exec) may execute attacker-"
            "controlled input. Use safer parsing libraries or restricted evaluators."
        ),
        "examples": ["eval(user_input)", "exec(code_str)"],
        "fix": (
            "Avoid eval/exec. Use safe interpreters, whitelist allowed operations, "
            "or parse inputs with a structured parser."
        ),
        "references": ["https://owasp.org/www-community/"],
    },
    "deprecated hash": {
        "summary": (
            "Weak hashing algorithms like MD5 or SHA1 are unsuitable for security-"
            "sensitive purposes. Use SHA-2/3 or password-specific KDFs instead."
        ),
        "examples": ["hashlib.md5(data).hexdigest()"],
        "fix": (
            "Use hashlib.sha256 or a password KDF such as bcrypt, scrypt, or "
            "argon2 for passwords. Migrate stored hashes where possible."
        ),
        "references": [
    ...
    "suspicious subprocess call": {
        "summary": (
            "Using subprocess APIs with shell=True or untrusted string inputs can "
            "lead to command injection. Pass args as a list and validate inputs."
        ),
        "examples": ["subprocess.run(cmd, shell=True)", "os.system(user_input)"],
        "fix": (
            "Avoid shell=True. Pass arguments as lists, validate or canonicalize "
            "inputs, and prefer high-level libraries."
        ),
        "references": ["https://cheatsheetseries.owasp.org/"],
    },
    "insecure regex": {
        "summary": (
            "Overly-broad regex patterns can cause catastrophic backtracking and "
            "unintended matches. Use anchors and limit quantifiers."
        ),
        "examples": ["re.match('(a+)+b', s)", "unbounded quantifiers"],
        "fix": (
            "Use non-backtracking engines, add bounds to quantifiers, or use "
            "safer parsing logic."
        ),
        "references": ["https://owasp.org/www-community/"],
    },
    "broken access control": {
        "summary": (
            ...
            "permissions. Always enforce authorization checks on the server side."
        ),
        "examples": ["/users/1234/profile accessible to other users"],
        "fix": (
            "Enforce authorization checks consistently on the server, use "
            "object-level ACLs, and avoid relying on client-side controls."
        ),
        "references": [
    ...
    "cross site scripting": {
        "summary": (
            "Cross-Site Scripting (XSS) occurs when applications include untrusted "
            "data in web pages without proper validation or escaping."
        ),
        "examples": ["document.write(user_input)", "innerHTML = user_content"],
        "fix": (
            "Escape output, use context-aware encoding, and use CSP headers. "
            "Prefer safe templating engines that auto-escape."
        ),
        "references": [
    ...
    "security misconfiguration": {
        "summary": (
            ...
            "default credentials, open cloud storage, and unnecessary features "
            "enabled in production."
        ),
        "examples": ["debug mode enabled in production", "default admin passwords"],
        "fix": (
            "Harden defaults, disable debug endpoints, rotate credentials, "
            "and enforce secure defaults in CI/CD."
        ),
        "references": ["https://owasp.org/www-project-top-ten/"],
    },
    "insufficient logging": {
        "summary": (
            "Insufficient logging and monitoring delays detection and response to "
            "security incidents. Ensure important events are logged and monitored."
        ),
        "examples": ["no audit log for failed login attempts"],
        "fix": (
            "Log security-relevant events, centralize logs, and configure "
            "alerts for suspicious activity. Avoid logging secrets."
        ),
        "references": ["https://cheatsheetseries.owasp.org/"],
    },
    ...
    "open redirect": {
        "summary": (
            ...
            "to untrusted URLs based on user input."
        ),
    ...
    "insecure deserialization": {
        "summary": (
            "Deserializing untrusted data can lead to remote code execution or "
            "state manipulation."
        ),
        "examples": ["pickle.loads(untrusted_bytes)"],
        "fix": "Avoid insecure serializers, use safe formats (JSON) and explicit schema validation.",
        "references": [
            "https://cheatsheetseries.owasp.org/cheatsheets/Insecure_Deserialization_Cheat_Sheet.html",
        ],
    },
    "unencrypted sensitive data": {
        "summary": (
            "Sensitive data in transit or at rest should be encrypted using "
            "modern algorithms and TLS."
        ),
    ...
    "insufficient input validation": {
        "summary": (
            "Failing to validate input allows many classes of bugs and security "
            "issues (injection, overflow, logic bugs)."
        ),
        "fix": (
            "Validate inputs against an allowlist, enforce types and length limits, "
            "and use schema validation libraries."
        ),
        "references": ["https://cheatsheetseries.owasp.org/"],
    },
    "server side request forgery": {
        "summary": (
            ...
            "services. Often arises when URLs are user-controlled."
        ),
        "examples": ["fetch(user_supplied_url)"],
        "fix": (
            "Validate and restrict outbound requests, use allowlists and timeouts, "
            "and isolate network access for fetchers."
        ),
        "references": ["https://cheatsheetseries.owasp.org/"],
    },
    ...
    "path traversal": {
        "summary": (
            "If file paths are constructed from user input, attackers can traverse "
            "directories and access unintended files."
        ),
    ...
    "insecure cookie": {
        "summary": (
            "Cookies missing Secure/HttpOnly/SameSite flags can be exposed to "
            "JavaScript or sent over insecure channels."
        ),
    ...
    "clickjacking": {
        "summary": (
            "Clickjacking allows UI redressing attacks by embedding the site in "
            "frames on attacker-controlled pages."
        ),
    ...
    "broken cryptography": {
        "summary": (
            "Using weak algorithms, improper key sizes, or insecure modes can "
            "break confidentiality or integrity."
        ),
        "fix": (
            "Use vetted cryptographic libraries, follow current crypto best "
            "practices, and prefer AEAD modes (e.g., AES-GCM)."
        ),
        "references": [
            "https://cheatsheetseries.owasp.org/cheatsheets/Cryptographic_Storage_Cheat_Sheet.html",
        ],
    ...
    "large file upload": {
        ...
        "fix": (
            "Enforce file size limits, validate content types, scan files for "
            "malware, and store uploads outside the web root."
        ),
        "references": ["https://cheatsheetseries.owasp.org/"],
    },
    "insecure cors": {
        "summary": (
            "Broad CORS policies (e.g., Access-Control-Allow-Origin: *) can "
            "expose APIs to untrusted web origins."
        ),
    ...
    "exposed debug endpoint": {
        "summary": (
            "Debug endpoints (debug toolbar, REPLs) should never be enabled "
            "in production as they expose internals."
        ),
    ...
    "missing security headers": {
        "summary": (
            "Absence of headers like Content-Security-Policy, "
            "X-Content-Type-Options, and Strict-Transport-Security lowers protection."
        ),
    ...
    "insecure direct object reference": {
        "summary": (
            "Insecure Direct Object Reference (IDOR) occurs when user-controlled "
            "identifiers allow access to resources belonging to others."
        ),
        "examples": ["/invoices/12345 where IDs are guessable"],
        "fix": (
            "Enforce object-level authorization checks and use non-guessable "
            "identifiers or access controls on the server side."
        ),
        "references": ["https://cheatsheetseries.owasp.org/"],
    },
    "server side template injection": {
        "summary": (
            "Server-Side Template Injection (SSTI) arises when untrusted input is "
            "evaluated by a template engine, potentially leading to RCE."
        ),
        "examples": ["render_template(user_input)", "{{7*7}}"],
        "fix": (
            "Do not evaluate user input in templates. Use safe templating engines "
            "and escape or whitelist variables."
        ),
        "references": ["https://cheatsheetseries.owasp.org/"],
    },
    ...
    "insecure mobile storage": {
        "summary": (
            "Storing secrets on mobile devices without encryption or OS-"
            "provided secure storage can leak credentials."
        ),
    ...
    "certificate pinning missing": {
        "summary": (
            "Mobile or embedded clients that don't pin TLS certificates can be susceptible to "
            "MITM via rogue CAs."
        ),
        "fix": (
    ...
    "backup credentials stored in repo": {
        "summary": (
            "Credentials accidentally checked into repositories in backup files lead to "
            "long-lived secrets exposure."
        ),
        "fix": (
    ...
    "missing content security policy": {
        "summary": (
            "Not setting a Content-Security-Policy allows more vectors for XSS and resource "
            "injection."
        ),
        "fix": (
