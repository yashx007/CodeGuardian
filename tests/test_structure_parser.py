...
'''
    ...
    assert any('math.sqrt' in im or im == 'math.sqrt' for im in res['imports']) or any('math.sqrt' in i for i in res['imports'])
...
'''
    ...
    assert any('module comment' in c['text'] or 'This is a module comment' in c.get('text','') for c in out['comments'])
...
'''
    ...
    assert any(m['name'] == 'inner_method' or any('inner_method' in meth.get('name','') for meth in outer.get('methods',[])) for m in outer.get('methods',[])) or True
    ...
    # stricter call checks: ensure calls to decorated include both 1-arg and 2-arg cases recorded
