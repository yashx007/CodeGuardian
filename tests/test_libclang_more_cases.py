...
def test_libclang_handles_missing_includes(tmp_path: Path):
    # source references a header that doesn't exist on the system; libclang should still parse enough to find calls
    code = '''
...
'''
    ...
    # re-importing function to pick up env change isn't necessary; function reads env at import time
    issues = parser.analyze_cpp_with_clang(str(p))
    for it in issues:
        if it.get('type') == 'Dangerous Function':
            # snippet should be present and include context lines when context>0
            assert 'snippet' in it
            snip = it.get('snippet') or ''
            assert snip.strip() != ''
            # with context=2 we expect at least two lines in the snippet in most cases
